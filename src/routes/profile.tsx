...
function ProfilePage() {
      ...
      <div className="min-h-screen" style={{ background: "var(--bg-main)" }}>
        ...
        <div className="flex items-center justify-center min-h-[70vh]">
          <div className="text-center">
            ...
            <h2 className="font-display font-bold text-xl mb-2" style={{ color: "var(--text-primary)" }}>No profile yet</h2>
            <button onClick={() => navigate({ to: "/discover" })} className="px-6 py-3 rounded-xl text-sm font-semibold mt-4"
  ...
  const examLabel = ACADEMIC_FOCUS.find(a => a.value === profile.examFocus?.[0])?.label || profile.examFocus?.[0] || "";
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6 pb-16">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>Display name</label>
            ...
            <div>
              ...
              <input type="number" value={profile.age} onChange={e => update({ age: parseInt(e.target.value) || 18 })} min={18} max={40}
                className="w-full px-4 py-3 rounded-xl border text-sm outline-none"
                style={{ background: "var(--bg-card)", borderColor: "var(--hairline)", color: "var(--text-primary)" }} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>College</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={profile.examFocus?.[0] || ""} onChange={e => update({ examFocus: [e.target.value] })} placeholder="e.g. B.Tech Sem 4, JEE, etc."
                className="w-full px-4 py-3 rounded-xl border text-sm outline-none"
                style={{ background: "var(--bg-card)", borderColor: "var(--hairline)", color: "var(--text-primary)" }} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>Career goal</label>
              <input value={profile.careerGoal} onChange={e => update({ careerGoal: e.target.value })} placeholder="e.g. Software Engineer, Doctor"
                className="w-full px-4 py-3 rounded-xl border text-sm outline-none"
                style={{ background: "var(--bg-card)", borderColor: "var(--hairline)", color: "var(--text-primary)" }} />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={profile.bio} onChange={e => update({ bio: e.target.value })} rows={4} maxLength={300}
          ...
          {/* Looking for */}
          <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>Looking for</label>
            <input value={profile.lookingForPrompt} onChange={e => update({ lookingForPrompt: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border text-sm outline-none"
              style={{ background: "var(--bg-card)", borderColor: "var(--hairline)", color: "var(--text-primary)" }} />
          </div>
          ...
          <div className="p-5 rounded-2xl border" style={{ borderColor: "var(--hairline)", background: "var(--bg-card)" }}>
            ...
            <input value={profile.studentEmail || ""} onChange={e => update({ studentEmail: e.target.value })}
            ...
            {profile.studentEmail && (
              <p className="text-xs" style={{ color: isStudentEmail(profile.studentEmail) ? "#10B981" : "#EF4444" }}>
            ...
            {profile.isVerified && (
              <div className="flex items-center gap-2 mt-3 px-3 py-2 rounded-xl" style={{ background: "rgba(16,185,129,0.1)" }}>
                <span className="text-sm">🎓</span>
                <span className="text-xs font-medium" style={{ color: "#10B981" }}>Campus Verified — Get Campus plan pricing!</span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 rounded-2xl border text-center" style={{ borderColor: "var(--hairline)", background: "var(--bg-card)" }}>
              <span className="text-2xl font-display font-bold block" style={{ color: "var(--rose-accent)" }}>{profile.hoursStudied}</span>
              <span className="text-[11px]" style={{ color: "var(--text-muted)" }}>Hours studied</span>
            </div>
            <div className="p-4 rounded-2xl border text-center" style={{ borderColor: "var(--hairline)", background: "var(--bg-card)" }}>
              <span className="text-2xl font-display font-bold block" style={{ color: "var(--rose-accent)" }}>{profile.streak}</span>
          ...
          <button onClick={handleSave}
            ...
            style={{ background: saved ? "#10B981" : "var(--rose-accent)", color: saved ? "#fff" : "#0B1120" }}>
