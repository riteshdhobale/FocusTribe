      ...
      <section className="relative pt-32 pb-24 px-6 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto text-center">
          ...
          {inReverseTrial && trialDaysLeft > 0 && (
            <div className="mb-8 inline-flex items-center gap-2 px-5 py-2.5 rounded-full border text-sm animate-fade-up"
              style={{ borderColor: "rgba(255,107,158,0.4)", background: "rgba(255,107,158,0.08)", color: "#FF6B9E" }}>
              <Sparkles className="w-4 h-4" />
              <span>You're on a <strong>free Pro trial</strong> — {trialDaysLeft} days left. Upgrade to keep your streak!</span>
            </div>
          )}

          {/* Payment feedback */}
          {paymentSuccess && (
            <div className="mb-8 inline-flex items-center gap-2 px-5 py-3 rounded-2xl border text-sm animate-fade-up"
              style={{ borderColor: "rgba(16,185,129,0.4)", background: "rgba(16,185,129,0.08)", color: "#10B981" }}>
              <CheckCircle2 className="w-5 h-5" />
              <span><strong>Payment successful!</strong> Welcome to StudyDate Pro. Your subscription is now active.</span>
            </div>
          )}

          {paymentError && paymentError !== "Payment cancelled" && (
            <div className="mb-8 inline-flex items-center gap-2 px-5 py-3 rounded-2xl border text-sm animate-fade-up cursor-pointer"
              onClick={clearError}
              style={{ borderColor: "rgba(239,68,68,0.4)", background: "rgba(239,68,68,0.08)", color: "#EF4444" }}>
              <AlertCircle className="w-5 h-5" />
              <span>{paymentError} <span className="underline opacity-60 ml-1">Dismiss</span></span>
            </div>
          )}

          <h1 className="font-display font-extrabold text-4xl md:text-5xl md:text-[3.5rem] tracking-tight leading-tight mb-6">
            Stop paying for small talk.<br />
          ...
          <div className="mb-20 surface-card p-8 md:p-10 rounded-[32px] relative overflow-hidden" style={{ borderColor: "rgba(255,107,158,0.15)" }}>
            ...
            <div className="relative z-10">
              <p className="text-[10px] font-mono tracking-widest uppercase text-center mb-2" style={{ color: "var(--rose-accent)" }}>
                Put it in perspective
              </p>
              <h2 className="text-2xl md:text-3xl font-display font-bold text-center mb-8" style={{ color: "var(--text-primary)" }}>
                You already pay to <span style={{ opacity: 0.6 }}>consume</span>.<br className="hidden md:block" /> What if you paid the same to <span className="text-rose-gradient">actually grow</span>?
              </h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                {comparisons.map((item) => (
                  <div key={item.app} className="p-6 rounded-2xl border transition-all duration-300 group hover:-translate-y-1 flex flex-col h-full"
                    ...
                    <div className={`text-2xl font-display font-extrabold ${item.highlight ? "text-rose-gradient" : ""}`}>
                      {item.price}<span className="text-xs font-normal text-[color:var(--text-muted)]">{item.per}</span>
                    </div>
                    <div className="text-sm font-bold mt-2 mb-3" style={{ color: item.highlight ? "#FF6B9E" : "var(--text-primary)" }}>{item.app}</div>
                    
                    <div className="mt-auto pt-3 border-t" style={{ borderColor: item.highlight ? "rgba(255,107,158,0.2)" : "var(--hairline)" }}>
                      <div className={`text-xs font-semibold leading-snug ${item.type === "waste" ? "text-red-400/80" : "text-[#FF6B9E]"}`}>
                        {item.note}
                      </div>
                    </div>
                  </div>
              ...
              <p className="text-center text-sm md:text-base mt-10 max-w-2xl mx-auto leading-relaxed" style={{ color: "var(--text-secondary)" }}>
                Three of these drain your time. <span className="font-semibold" style={{ color: "#FF6B9E" }}>One gives it back.</span><br />
                <span className="text-xs md:text-sm mt-3 block px-4 py-3 rounded-xl border border-[color:var(--hairline)] bg-[color:var(--surface-2)]" style={{ color: "var(--text-primary)" }}>
                  💡 Think of it as your <span className="font-semibold text-rose-gradient">virtual co-working space</span>. Join focused sprints from your desk, your hostel, or your café. Real accountability with people who get it.
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 text-left">
            ...
            <div className="surface-card p-8 rounded-[24px] border border-[color:var(--hairline)] flex flex-col">
              ...
              <ul className="space-y-4 mb-8 flex-1">
                ...
                <li className="flex items-start gap-3 text-sm text-[color:var(--text-secondary)]">
                  <Check className="w-5 h-5 shrink-0" style={{ color: "#FF6B9E" }} /> All exam categories
                </li>
                <li className="flex items-start gap-3 text-sm text-[color:var(--text-secondary)]">
            ...
            <div className="surface-card p-8 rounded-[24px] border border-[color:var(--hairline)] flex flex-col relative">
              ...
              <button
                ...
                style={{ color: "#FF6B9E", borderColor: "rgba(255,107,158,0.3)", background: "rgba(255,107,158,0.05)" }}
