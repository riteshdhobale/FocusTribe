  ...
  errorComponent: ({ error }) => (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "var(--bg-main)" }}>
      <div className="text-center max-w-md">
        ...
        <h1 className="font-display font-bold text-2xl mb-2" style={{ color: "var(--text-primary)" }}>Loading StudyDate...</h1>
...
function DiscoverPage() {
  ...
  const { isBanned, reason: banReason, banType, expiresAt: banExpiry, loading: banLoading } = useBanCheck();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "var(--bg-main)" }}>
        <div className="text-center">
          <div className="w-10 h-10 rounded-xl mx-auto mb-3 flex items-center justify-center animate-pulse" style={{ background: "#FF6B9E" }}>
            <span className="font-bold" style={{ color: "#0B1120" }}>S</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8">
          ...
          <div className="flex flex-col items-center">
            <div className="flex items-center justify-between w-full max-w-md mb-6">
              <div>
                <span className="text-xs font-mono tracking-widest uppercase mb-1 block" style={{ color: "#FF6B9E" }}>Swipe Deck</span>
                <h1 className="font-display font-bold text-2xl" style={{ color: "var(--text-primary)" }}>Find your next study date</h1>
              </div>
            ...
            {currentProfile ? (
              ...
              <div className="w-full max-w-md p-10 rounded-[2rem] border relative overflow-hidden text-center group" style={{ borderColor: "rgba(255,107,158,0.2)", background: "linear-gradient(145deg, rgba(30,41,59,0.4) 0%, rgba(15,23,42,0.8) 100%)", backdropFilter: "blur(20px)" }}>
                ...
                <div className="relative z-10">
                  <div className="w-24 h-24 mx-auto mb-6 rounded-full flex items-center justify-center border-4 relative" style={{ borderColor: "rgba(255,107,158,0.1)", background: "rgba(11,17,32,0.5)" }}>
                  ...
                  <button onClick={() => { handlePrefsChange({ ...prefs, ageRange: null, cities: [], colleges: [], locationMode: "global" }); refreshDeck(); }}
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {prefs.locationMode !== "global" ? "🌍 Unlock Global Discovery" : "🔄 Reset Filters & Refresh"}
                    </span>
                  </button>
                </div>
              </div>
